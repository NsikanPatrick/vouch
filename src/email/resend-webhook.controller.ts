import { Controller, Post, Body, Logger, HttpCode, HttpStatus, Headers, Req } from '@nestjs/common';
...
export class ResendWebhookController {
    ...
    @HttpCode(HttpStatus.OK)
    async testWebhook() {
        this.logger.log('Test webhook endpoint hit!');
    ...
    private async handleEmailDelivered(event: ResendWebhookEvent) {
        ...
        this.logger.log(`Processing delivery for messageId: ${messageId}`);
        ...
        if (emailLog) {
            ...
            this.logger.warn(`Available messageIds in DB: ${await this.getRecentMessageIds()}`);
        }
    }

    private async handleEmailOpened(event: ResendWebhookEvent) {
        const messageId = event.data.email_id;
        this.logger.log(`Processing open for messageId: ${messageId}`);
        ...
        if (emailLog) {
            if (emailLog.status === EmailStatus.SENT || emailLog.status === EmailStatus.DELIVERED) {
                ...
                this.logger.log(`Email ${messageId} has status ${emailLog.status}, not updating to opened`);
            }
        } else {
            this.logger.warn(`Email log not found for messageId: ${messageId}`);
        }
    ...
    private async handleEmailClicked(event: ResendWebhookEvent) {
        const messageId = event.data.email_id;
        this.logger.log(`Processing click for messageId: ${messageId}`);
    ...
    private async handleEmailComplained(event: ResendWebhookEvent) {
        ...
        if (emailLog) {
            ...
            await this.emailLogRepository.save(emailLog);
            this.logger.log(`Email ${messageId} marked as complained`);
        } else {
            this.logger.warn(`Email log not found for messageId: ${messageId}`);
        }
    }

    private async getRecentMessageIds(): Promise<string[]> {
        const recentLogs = await this.emailLogRepository.find({
            select: ['messageId'],
            order: { createdAt: 'DESC' },
            take: 5,
        });
        return recentLogs.map(log => log.messageId);
    }
}
